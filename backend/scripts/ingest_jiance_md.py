...
# -*- coding: utf-8 -*-
"""
2026年6月采砂现场监测 Markdown → LlamaIndex 知识库导入脚本

切片规则与 ingest_caisha_md.py（潢川县实施方案）保持一致：
  - 编号段数+1 = 逻辑层级：  "1"→二级(章/县区)  "1.1"→三级(节/砂场，父块)
  - 三级标题(砂场/工程)  → 父块：保留 县区面包屑 + 县区引言 + 本节直属内容
  - 四/五级标题          → 子块（本文档暂无）
  - 二级标题下无编号子节 → 整章作单一父块
  - 图片：给 images/xxx 拼接前缀 http://172.136.16.52:82/ 渲染为可访问 URL 并存入元数据
  - 项目文件夹：xianchangjiance（现场监测报告）

本文档格式差异适配：
  - 标题含 ** 加粗符号（如 "## **1.1李湖砂场**"）→ 先剥离 * 再解析
  - 编号与标题间可能无分隔符（如 "1.1李湖砂场"、"5.固始县"）→ 正则放宽

用法：
  python scripts/ingest_jiance_md.py --dry-run     # 仅解析并导出 chunks JSON，不入库
  python scripts/ingest_jiance_md.py --rebuild      # 先删旧节点再入库并持久化
  python scripts/ingest_jiance_md.py                # 增量入库（不删旧）
"""
...
MD_PATH = os.environ.get(
    "JIANCE_MD_PATH", "/home/server/python/采砂/2026年6月采砂现场监测.md"
)
DOC_ID = "xinyang_202606_xianchangjiance"
DOC_TITLE = "2026年6月采砂现场监测报告"
# 项目“文件夹”（知识库分组标签）
PROJECT = "xianchangjiance"
PROJECT_NAME = "2025年现场监测报告"
PROJECT_YEAR = "2025"
IMAGE_BASE_URL = "http://172.136.16.52:82/"
OUTPUT_JSON = os.path.join(
    os.path.dirname(__file__), "..", "output", "2026年6月采砂现场监测_chunks.json"
)
...
# 放宽版：编号后允许无分隔符直接接中文标题（"1.1李湖砂场"、"5.固始县"）
...
def parse_document(lines):
    ...
    while i < n:
        ...
        if m:
            hashes, htext = m.group(1), clean_heading(m.group(2))
            if htext == "目录":
                current = new_section("skip", None, 99, htext, line, i + 1)
                i += 1
                continue
            numm = NUM_RE.match(htext)
            if numm and numm.group(2).strip():
                num = numm.group(1)
...
def build_chunks(sections):
    ...
    for sec in sections:
        ...
        if kind in ("cover", "preface"):
            label = "封面" if kind == "cover" else "前言"
...
def main():
    parser = argparse.ArgumentParser(description="2026年6月采砂现场监测 → LlamaIndex 知识库导入")
