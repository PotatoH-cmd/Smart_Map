...
/* ---- 服务端文件浏览器弹窗 ---- */
const FileBrowser = ({ visible, onClose, onSelect, extensions = '.tif,.tiff' }) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontWeight: 700, fontSize: 15, flex: 1 }}>选择 GeoTIFF 文件</span>
...
const TileManager = () => {
  ...
  const handleBuildSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showToast(response.data.geoserver ? `${response.data.layer} 已构建并发布到 GeoServer` : `${response.data.layer} 构建完成`);
        await fetchLayers();
        await fetchGeoServerLayers();
        await fetchGeoServerStatus();
        if (response.data.geoserver?.layer_name) {
          setGsExpanded(true);
          setGsPreviewLayer(response.data.geoserver.layer_name);
  ...
  const handleDroneDelete = async (layerKey, label) => {
    const ok = window.confirm(
      `确定要删除无人机影像「${label || layerKey}」吗？\n\n` +
      `勾选“同时删除文件”会从磁盘移除 MBTiles 数据，此操作不可恢复。`
    );
    if (!ok) return;
    const deleteFiles = window.confirm('是否同时删除磁盘上的 MBTiles / 工作文件？\n\n“确定”= 删除文件\n“取消”= 仅从注册表移除');
  ...
  /** 删除图层列表中的任意图层（矢量/栅格/无人机/虚拟） */
  const handleTileLayerDelete = async (layer) => {
    const { key, label, type } = layer;
    const isVirtual = layer.directory?.startsWith('PostGIS') || layer.source_path?.startsWith('postgres');
    const typeLabel = type === 'drone' ? '无人机影像' : isVirtual ? 'PostGIS 图层' : type === 'vector' ? '矢量图层' : '栅格图层';

    const ok = window.confirm(`确定要删除${typeLabel}「${label || key}」吗？`);
    if (!ok) return;

    let deleteFiles = false;
    if (isVirtual) {
      deleteFiles = window.confirm('是否同时删除 PostGIS 数据库中的表？\n\n“确定”= 删除数据表（不可恢复）\n“取消”= 仅从 GeoServer 取消发布');
    } else {
      deleteFiles = window.confirm('是否同时删除磁盘上的切片/源文件？\n\n“确定”= 删除文件\n“取消”= 仅从注册表移除');
    }

    setTileDeleteKey(key);
    setError(null);
    try {
      const apiUrl = type === 'drone'
  ...
  const handle3DTilesetDelete = async (dataset) => {
    const ok = window.confirm(
      `确定要删除 3D Tiles「${dataset.label || dataset.key}」吗？\n\n` +
      `勾选"同时删除文件"会从磁盘移除数据，此操作不可恢复。`
    );
    if (!ok) return;
    const deleteFiles = window.confirm('是否同时删除磁盘上的 3D Tiles 文件？\n\n"确定"= 删除文件\n"取消"= 仅从注册表移除');
    setThreeDDeleteKey(dataset.key);
    setError(null);
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/api/tile_manager/3dtiles/${encodeURIComponent(dataset.key)}`,
        { params: { delete_files: deleteFiles } }
      );
      if (response.data.success) {
        const removed = response.data.deleted_files || [];
        showToast(`已删除「${dataset.label || dataset.key}」${removed.length ? `，清理了 ${removed.length} 个文件` : ''}`);
        await fetch3DTilesets();
        await fetchLayers();
      } else {
        setError(response.data.error || '删除失败');
  ...
  return (
    <div className="tile-manager">
      ...
      <style jsx>{`
        ...
        .grid-drone-params {
          grid-template-columns: 110px 120px 120px 120px 120px 120px;
