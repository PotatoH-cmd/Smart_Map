...
class TaskExecutor:
    ...
    def _create_tool(self, tool_name: str):
        try:
            if tool_name == "map_tool":
                ...
                return WeatherTool()
            elif tool_name == "spatial_processing_tool":
    ...
    def _intent_node(self, state: AgentState) -> AgentState:
        ...
        logger.info(
            f"[intent_node] intent={intent_result.primary_intent}, "
    ...
    async def _tool_node(self, state: AgentState) -> AgentState:
        """节点2：并行调用 execution_plan 中的所有工具步骤。"""
        ...
        # ── 数据库空结果降级：自动回退到知识库检索 ──
        pre_results = await self._fallback_knowledge_search(
            pre_results, state["user_message"], _invoke_step
        )
        ...
        if report_steps:
            ...
            if not data_sufficient:
                ...
                for rs in report_steps:
                    auto_vars = self._build_report_variables(
                        ordered_pre, state["user_message"], intent_result
                    )
                    r = await _invoke_step(rs, extra_params={"variables": auto_vars})
                    report_results.append(r)

        state["tool_results"] = pre_results + report_results
        return state
    ...
    def _response_node(self, state: AgentState) -> AgentState:
        """节点3：汇总工具结果，生成最终回答，并提取 map/cesium/chart 数据。"""
        intent_result: IntentResult = state["intent_result"]
        ...
        for item in tool_results:
            ...
            if tool_name == "report_generator_tool" and result.get("success"):
                ...
                logger.info(f"[response_node] Report generated: report_url={report_url} (from tool_name={tool_name}, result keys={list(result.keys())})")
            ...
            if tool_name == "knowledge_base_tool" and result.get("success"):
                ...
                summary = f"[{tool_name}]: {str(content)[:500]}"
                # 对数据库查询，把 data 的 JSON 摘要也加进来，防止 LLM
                # 把"返回 1 条记录"（聚合查询）误解为只有 1 个数据点
        ...
        if not tool_summaries and intent_result.primary_intent in (
            ...
            if all_steps:
                logger.warning(
                    f"[response_node] Map intent '{intent_result.primary_intent}' "
                ...
                if isinstance(r, dict):
                    ...
                    if data is not None and (isinstance(data, list) and data):
                        ...
                        if self._is_empty_aggregate(data):
                            need_fallback = True
                            break
                        # 检查是否异常大数值（如 COUNT(*) 误用了测量点数）
                        for row in data:
                            if isinstance(row, dict):
                                for v in row.values():
                                    if isinstance(v, (int, float)) and v > 100:
                                        logger.warning(
                                            f"[response_node] Suspicious large count {v}, "
                            ...
                            if need_fallback:
                                break
            if need_fallback:
                logger.warning("[response_node] DB result unreliable — running sync fallback")
        ...
        if rich_content:
            logger.info("[response_node] Skipping LLM response generation — using rich tool content directly")
        ...
        logger.info(f"[response_node] Final state: report_url={report_url}, charts={len(charts)}, map_cmds={len(map_commands)}, response_len={len(response_text)}")
        return state
    ...
    def _try_extract_rich_response(self, tool_results: List[Dict]) -> Optional[str]:
        ...
        covered_tools = {"data_visualizer_tool", "report_generator_tool", "spatial_processing_tool"}
        has_viz = any(
    ...
    def _generate_response(
        ...
        if not tool_summaries:
            return f"已完成操作：{intent_result.task_context}"
...
请根据上述工具执行结果，用简洁专业的语言回答用户的问题。"""
        ...
        try:
            ...
            logger.error(f"[response_node] LLM generate response error: {e}")
...
请确认是否按此计划执行？"""
    ...
    async def execute_stream(self, user_message: str, chat_history: Optional[List[Dict]] = None, thread_id: str = "default") -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            if route == "response_node" and intent_result and intent_result.requires_confirmation:
                yield {
                    "type": "status",
                    "stage": "confirmation",
                    "title": "等待用户确认",
            ...
            state = self._response_node(state)
            yield {
                "type": "status",
                "stage": "done",
                "title": "最终回复已生成",
