...
export default function GisPipeline() {
  ...
  // ---- 切片发布联动（处理结果 → 切片管理发布） ----
  const [autoPublish, setAutoPublish] = useState(false);   // 处理完成后自动发布
  const [pubModalVisible, setPubModalVisible] = useState(false);
  ...
  const [pubProgress, setPubProgress] = useState(null); // {percent, message, stage}
  const [pubRunning, setPubRunning] = useState(false);
  const autoPublishTriggeredRef = useRef(false);

  // 流水线最终输出文件（镶嵌开启时存在）
  const pipelineOutputPath = doMosaic
    ? `${resolvePath(outDir).replace(/\/$/, '')}/${finalName}`
    : '';

  // 提交发布任务 + 轮询进度（自动/手动共用）
  const doPublish = async (body) => {
    setPubRunning(true);
    setPubProgress({ percent: 0, message: '正在提交发布任务...', stage: 'init' });
    try {
      const res = await fetch(`${API_BASE}/gis-tool/publish-tile`, {
        method: 'POST',
      ...
      if (!res.ok || !d.success) {
        setPubRunning(false);
        setPubProgress(null);
        alert(d.detail || d.message || '发布提交失败');
        return false;
      }
      const jobId = d.job_id;
      const timer = setInterval(async () => {
        try {
          const sres = await fetch(`${API_BASE}/api/tile_manager/build_status/${jobId}`);
          const sdata = await sres.json();
          const job = sdata.job;
          setPubProgress({
            percent: job.percent || 0,
            message: job.message || '发布中...',
          ...
          if (job.done) {
            ...
            setPubRunning(false);
            if (job.success) {
              setPubProgress({ percent: 100, message: `发布完成：图层「${d.layer_key}」已就绪`, stage: 'done' });
              setTimeout(() => setPubProgress(null), 8000);
            } else {
              setPubProgress(null);
              alert(`发布失败：${job.message || '未知错误'}`);
            }
          }
        } catch (e) {
          clearInterval(timer);
  ...
  // 打开发布弹窗（预填默认输出路径）
  const openPublishModal = (pathOverride = '') => {
    const src = pathOverride || pipelineOutputPath;
    const ext = src.toLowerCase();
    const baseName = src ? src.split('/').pop().replace(/\.[^.]+$/, '') : '';
    setPubForm(prev => ({
      ...prev,
      sourcePath: src,
      layerKey: baseName,
      name: baseName,
      layerType: ext.endsWith('.geojson') || ext.endsWith('.json') ? 'vector' : 'auto',
    }));
    setPubModalVisible(true);
  };

  // 手动发布（弹窗表单）
  const submitPublish = async () => {
    if (!pubForm.sourcePath.trim()) {
      alert('请填写要发布的文件路径');
      return;
    }
    const ok = await doPublish(pubForm);
    if (ok) setPubModalVisible(false);
  };

  // 自动发布（处理完成后触发，路径已知时直接发布）
  const autoTriggerPublish = () => {
    if (!pipelineOutputPath) {
      openPublishModal(); // 无法确定输出文件，弹出让用户填写
      return;
    }
    const baseName = pipelineOutputPath.split('/').pop().replace(/\.[^.]+$/, '');
    const body = {
      ...pubForm,
      sourcePath: pipelineOutputPath,
      layerKey: pubForm.layerKey || baseName,
      name: pubForm.name || baseName,
    };
    doPublish(body);
  };
    ...
    logTimerRef.current = setInterval(async () => {
      try {
        ...
        if (data.status === 'finished') {
          ...
          // 勾选了自动发布 → 任务结束后自动发布结果
          if (autoPublish && !autoPublishTriggeredRef.current) {
            autoPublishTriggeredRef.current = true;
            setTimeout(() => autoTriggerPublish(), 300);
          }
        }
      } catch (e) {
  ...
  const submitPipeline = async () => {
    ...
    autoPublishTriggeredRef.current = false; // 新任务允许再次自动发布
  ...
  // 全选当前可见的 TIF 文件
  const selectAllVisibleTif = () => {
    const selector = fileModalMode === 'be' ? '.be-file-check' : '.file-check';
    document.querySelectorAll(selector).forEach(cb => {
      cb.checked = true;
      if (fileModalMode !== 'be') {
        browseSelectedRef.current.add(cb.value);
      }
    });
  };

  // 过滤文件列表
  const filteredItems = fileModalItems.filter(item => {
    if (!fileSearch.trim()) return true;
    if (item.type === 'dir') return item.name.toLowerCase().includes(fileSearch.toLowerCase());
    // 只对 tif/tiff 文件进行搜索过滤
    const isTif = item.name.toLowerCase().endsWith('.tif') || item.name.toLowerCase().endsWith('.tiff');
    return isTif && item.name.toLowerCase().includes(fileSearch.toLowerCase());
  });
  ...
  return (
    <div className="gis-pipeline">
      ...
      <div className="gis-pipeline-left">
        <h1 className="gis-pipeline-title">GIS 流水线引擎</h1>
        ...
        {/* 发布联动：处理结果 → 切片管理发布 */}
        <div className="gis-config-card gis-step-gold">
          <div className="gis-step-row">
            <label className="gis-step-check">
              <input type="checkbox" checked={autoPublish} onChange={(e) => setAutoPublish(e.target.checked)} />
              &nbsp;📦 处理完成后自动发布为切片图层
            </label>
            <span className="gis-input-hint-dim">任务结束自动生成地图瓦片，地图可直接加载</span>
          </div>
          <div className="gis-grid-3">
            <div>
              <label className="gis-label-pro">图层名 (key)</label>
              <input type="text" className="gis-input-pro" value={pubForm.layerKey}
                placeholder="默认取输出文件名"
                onChange={(e) => setPubForm(prev => ({ ...prev, layerKey: e.target.value }))} />
            </div>
            <div>
              <label className="gis-label-pro">最小缩放</label>
              <input type="number" className="gis-input-pro" value={pubForm.minZoom}
                onChange={(e) => setPubForm(prev => ({ ...prev, minZoom: Number(e.target.value) }))} />
            </div>
            <div>
              <label className="gis-label-pro">最大缩放</label>
              <input type="number" className="gis-input-pro" value={pubForm.maxZoom}
                onChange={(e) => setPubForm(prev => ({ ...prev, maxZoom: Number(e.target.value) }))} />
            </div>
          </div>
          {pubProgress && (
            <div className="gis-publish-progress">
              <div className="gis-publish-progress-bar" style={{ width: `${pubProgress.percent || 0}%` }} />
              <span className="gis-publish-progress-text">{pubProgress.message}</span>
            </div>
          )}
        </div>
      ...
      <div className="gis-pipeline-right">
        ...
        <div className="gis-terminal">
          <div className="gis-terminal-header">
            <span className="gis-terminal-label">终端日志</span>
            <span className="gis-terminal-taskid">ID: {currentTaskId || 'NONE'}</span>
          </div>
          <pre className="gis-terminal-content">{logContent}</pre>
          {!taskRunning && currentTaskId && logContent.includes('DONE:') && !autoPublish && (
            <button className="gis-btn-publish" onClick={() => openPublishModal()} disabled={pubRunning}>
              📦 发布为切片图层
            </button>
          )}
        </div>
        ...
        <div className="gis-modal-overlay" onClick={() => setFileModalVisible(false)}>
          <div className="gis-modal" onClick={(e) => e.stopPropagation()}>
            <div className="gis-modal-body">
              ...
              {/* 搜索与全选栏 */}
              <div className="gis-modal-search-bar">
                <input
                  type="text"
                  className="gis-input-pro gis-search-input"
                  placeholder="🔍 搜索 .tif 文件..."
                  value={fileSearch}
                  onChange={(e) => setFileSearch(e.target.value)}
                />
                <button className="gis-btn-xs gis-btn-purple" onClick={selectAllVisibleTif}>
                  ☑ 全选TIF
                </button>
              </div>
      ...
      {/* 切片发布弹窗 */}
      {pubModalVisible && (
        <div className="gis-modal-overlay" onClick={() => setPubModalVisible(false)}>
          <div className="gis-modal" onClick={(e) => e.stopPropagation()}>
            <div className="gis-modal-body">
              <div className="gis-modal-path">📦 发布为切片图层</div>
              <div style={{ marginBottom: 10 }}>
                <label className="gis-label-pro">文件路径（服务器上的 GeoTIFF / GeoJSON）</label>
                <input type="text" className="gis-input-pro" value={pubForm.sourcePath}
                  onChange={(e) => setPubForm(prev => ({ ...prev, sourcePath: e.target.value }))} />
              </div>
              <div className="gis-grid-2">
                <div>
                  <label className="gis-label-pro">图层名 (key)</label>
                ...
                <div>
                  <label className="gis-label-pro">显示名称</label>
                  <input type="text" className="gis-input-pro" value={pubForm.name}
                    onChange={(e) => setPubForm(prev => ({ ...prev, name: e.target.value }))} />
                </div>
              </div>
              <div className="gis-grid-3">
                <div>
                  <label className="gis-label-pro">类型</label>
                  <select className="gis-input-pro" value={pubForm.layerType}
                    onChange={(e) => setPubForm(prev => ({ ...prev, layerType: e.target.value }))}>
              ...
              <div className="gis-input-hint" style={{ marginTop: 8 }}>
                栅格将转成 MBTiles 瓦片，矢量将用 tippecanoe 生成 PBF 切片，发布后可在切片管理 / 地图中加载。
              </div>
            </div>
            <div className="gis-modal-footer">
              <button className="gis-btn-modal-confirm" onClick={submitPublish} disabled={pubRunning}>
                {pubRunning ? '发布中...' : '🚀 开始发布'}
              </button>
            </div>
          </div>
